    ...
    tokio::spawn(async move {
        if should_refresh_cfproxy_domains() {
            ...
            if CFPROXY.read().domains.is_empty() {
                ldebug!(
                    " CF: обновить список доменов не удалось, остаюсь на кеше/встроенном списке"
                );
            }
        }
        loop {
...
pub async fn try_refresh_cfproxy_domains() -> bool {
    ...
    let nonce: String = {
        ...
        (0..7).map(|_| rng.gen_range(b'a'..=b'z') as char).collect()
    };
    ...
    if !fetched_raw.is_empty() {
        ...
        ldebug!(
            " CF: обновить список доменов не удалось (пустой ответ), остаюсь на кеше/встроенном списке"
        );
    }
    false
}
